...
const searchManga = async () => {
    ...
    if (mangas.activeFilter) {
        if (category.value === 'top-rated' || category.value === 'most-popular') {
            await axios.post(`https://bakteria.online:5000/api/users/top-rated?start=${pagination.start}&end=${pagination.end}`, {
                hidden: user.hidden
            })
                .then((response) => {
            ...
            //No filter request
            await axios.post(`https://bakteria.online:5000/api/users/category/${category.value}?start=${pagination.start}&end=${pagination.end}`, {
                hidden: user.hidden
            })
                .then((response) => {
        ...
        if (category.value === 'top-rated' || category.value === 'most-popular') {
            await axios.get(`https://bakteria.online:5000/api/mangas/${category.value}?start=${pagination.start}&end=${pagination.end}`)
                .then((response) => {
            ...
            //No filter request
            await axios.get(`https://bakteria.online:5000/api/mangas/category/${category.value}?start=${pagination.start}&end=${pagination.end}`)
                .then((response) => {
                    //establecer los mangas como arreglos temporales
                    mangas.notFilter = response.data
                    //compriobar si se reinicia la paginacion  
